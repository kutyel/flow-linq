...
class List<T> {
  ...
  skipWhile(
    ...
    return this.skip(
      this.aggregate(x => (predicate(this.elementAt(x)) ? ++x : x), 0)
    );
  }
...
class OrderedList<T> extends List<T> {
  ...
  thenBy(keySelector: (key: T) => any): List<T> {
    return new OrderedList(
      this._elements,
      composeComparers(this._comparer, comparerForKey(keySelector, false))
    );
  }
  ...
  thenByDescending(keySelector: (key: T) => any): List<T> {
    return new OrderedList(
      this._elements,
      composeComparers(this._comparer, comparerForKey(keySelector, true))
    );
  }
}
